...
5 seconds) to AWS IoT Core, the valid data will be sent to a DynamoDB "Temperature" table and the invalid data (with a
temperature > 40 °C which is caused by captors artefacts) will be sent to another "InvalidTemperature" DynamoDB table.

Every minute an AWS Lambda function will retrieve the temperature data from the last minute from the Dynamo DB,
...
named "InvalidTemperature" with a *hash_key* named "id" an attribute named "id" of type string ("S")
...
the sql query "SELECT *, parse_time(\"yyyy/MM/dd'T'HH:mm:ss\", timestamp()) as time FROM 'sensor/temperature/+' where temperature > 40" 
and a **dynamodbv2** action to link to the InvalidTemperature table and linked to the "iot_role" role
...
grant the "InvalidTemperature" dynamoDB (**dynamodb:PutItem** action) access to the iam role "iot_role"
...
### Part 3: DynamoDB + Iot Topic rule (Valid Data)

![img7.png](doc/img_7.png)


In this part we are going to create another DynamoDB Table to store *valid data* and an IoT Topic Rule to forward mqtt messages to
the DB

We'll need to declare some *AWS Iot Core resources*  :

in the *dynamo.tf* file, an [**aws_dynamodb_table**](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/dynamodb_table)
named "Temperature" with a *hash_key* named "id", an attribute named "id" of type string ("S"), a *range_key* named "time" with an attribute named "time" of type string ("S")

in the *iot.tf* file, an [**aws_iot_topic_rule**](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iot_topic_rule) with
the sql query "SELECT *, parse_time(\"yyyy/MM/dd'T'HH:mm:ss\", timestamp()) as time FROM 'sensor/temperature/+' where temperature <= 40" 
and a **dynamodbv2** action to link to the Temperature table and linked to the "iot_role" role


* an [**aws_iam_role_policy**](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) to
grant the "Temperature" dynamoDB (**dynamodb:PutItem** action) access to the iam role "iot_role"
...
```
then you can go to the Explore items table for the "Temperature" table and see the entry.
...
one to grant **dynamoDB:Scan** action to the *Temperature* dynamoDB table we created earlier.
...
the *handler* **"ac_control_lambda.lambda_handler"**, the *runtime* **"python3.9"** and the *timeout* **30** seconds
